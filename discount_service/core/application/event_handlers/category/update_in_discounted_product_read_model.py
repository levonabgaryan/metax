...
class UpdateCategoryInDiscountedProductReadModel(EventHandler[CategoryUpdated]):
    async def handle_event(self, event: CategoryUpdated) -> None:
        updated_category = await self._unit_of_work.category_repo.get_by_uuid(event.category_uuid)
        await self._unit_of_work.discounted_product_read_model_repo.update_category(updated_category)
