...
from backend.core.application.use_cases.category.utils import process_get_category_by_name
...
class DeleteCategoryHelperWordsUseCase:
    ...
    async def execute(self, request: DeleteCategoryHelperWordsRequest) -> Result[EmptyResponse]:
        category_result = await process_get_category_by_name(self.category_repository, request.category_name)
        ...
        updated_result = await self.__process_deletion_of_words_and_save(category, request.words_to_delete)
    ...
    async def __process_deletion_of_words_and_save(
        self,
        category: Category,
