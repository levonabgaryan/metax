...
async def test_update_helper_word_text_service(
    ...
    assert response_dto.helper_words_payload.helper_word_text == "updated_helper_word_text"
    ...
    async with uow:
        ...
        updated_words = {word.get_helper_word_text() for word in updated_category.get_helper_words()}
        assert updated_category.get_name() == category.get_name()
        assert updated_words == {"updated_helper_word_text", helper_word_to_keep.get_helper_word_text()}
        await uow.commit()
