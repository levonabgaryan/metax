...
@pytest.mark.asyncio
async def test_event_handler_shall_delete_category_fragment_in_read_model(
    ...
    event = CategoryDeleted(category_uuid=category.get_uuid())
    ...
    # then
    updated_read_model = await discounted_product_read_model_repository.get_by_uuid(
        uuid_=str(discounted_product.get_uuid())
    )
    assert "category" not in updated_read_model
