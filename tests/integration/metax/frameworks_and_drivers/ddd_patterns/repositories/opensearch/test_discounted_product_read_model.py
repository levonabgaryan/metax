...
async def test_delete_older_than_and_return_deleted_count(
    ...
    deleted_count = await repo.delete_older_than_and_return_deleted_count(
        date_limit=creation_date + dt.timedelta(days=1)
    )
    await refresh_opensearch_index(metax_lifespan_manager_for_tests, discounted_product_read_model.ALIAS_NAME)
...
@pytest.mark.asyncio
async def test_search_by_name_and_by_category_uuid(
    metax_lifespan_manager_for_tests: MetaxAppLifespanManager,
) -> None:
    # given — same full-text query matches wines in category A (2 docs) and B (1 doc); noise without category.
    metax_container = metax_lifespan_manager_for_tests.get_metax_container()
    repo = await metax_container.get_discounted_product_read_model_repository()
    created_at = dt.datetime.now(tz=dt.UTC)
    category = make_category_entity(name="Ալկոհոլ")
    wine_1 = make_discounted_product_read_model(
        created_at=created_at,
        discounted_product_uuid=str(uuid.uuid7()),
        name="Փրփրուն գինի «Blue Nun Gold Edition» 0.75լ",
        category_uuid=str(category.get_uuid()),
    ...
    wine_2 = make_discounted_product_read_model(
        created_at=created_at,
        discounted_product_uuid=str(uuid.uuid7()),
        name="Գինի «Blue Nun Gold Edition» 0.5լ — երկրորդ տարաձև",
        category_uuid=str(category.get_uuid()),
    ...
    noise_products = [  # without categories
        make_discounted_product_read_model(
            created_at=created_at,
            discounted_product_uuid=str(uuid.uuid7()),
        )
    ...
    await repo.add_many([*noise_products, wine_1, wine_2])
    await refresh_opensearch_index(metax_lifespan_manager_for_tests, discounted_product_read_model.ALIAS_NAME)

    discounted_product_name = "Blue Nun"

    # when
    (
        discounted_products_with_category,
        total_discounted_products_count_with_category,
    ) = await repo.search_by_name_and_by_category_uuid(
    ...
    #  then
    assert total_discounted_products_count_with_category == 2
    assert {p["uuid_"] for p in discounted_products_with_category} == {wine_1["uuid_"], wine_2["uuid_"]}
    assert all(dp["category"]["name"] == "Ալկոհոլ" for dp in discounted_products_with_category)
    assert all(dp["category"]["uuid_"] == str(category.get_uuid()) for dp in discounted_products_with_category)
