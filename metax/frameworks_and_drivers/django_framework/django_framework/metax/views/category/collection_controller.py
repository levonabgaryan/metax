...
class CategoryCollectionController(MetaxJsonApiController):
    @modify(status_code=HTTPStatus.OK, tags=["Category"])
    async def get(self, parsed_query: Query[QueryParamsForCollection]) -> CategoryListResponseBody:
        ...
        response_body = CategoryListResponseBody.from_basemodel_list(resources=resources)
    ...
    async def post(
        ...
        response_dto = await CreateCategoryService(
            unit_of_work_provider=unit_of_work_provider, event_bus=event_bus
        ).execute(CreateCategoryRequestDTO(name=parsed_body.data.attributes.name))
