...
class MetaxContainer:
    """Application DI root: a small facade over the wired ``dependency_injector`` graph.

    Instantiating a ``DeclarativeContainer`` subclass yields a ``DynamicContainer`` that only
    exposes providers, not custom methods. This class holds that instance and exposes typed
    ``get_*`` helpers and resource init/shutdown for the rest of the app.
    """
