...
class DjangoPostgresqlCategoryRepository(CategoryRepository):
    ...
    async def all(self) -> list[Category]:
        def _sync_version() -> list[Category]:
            ...
            category_map: dict[
                UUID,
                tuple[CategoryName, dt.datetime, dt.datetime, list[CategoryHelperWord]],
            ] = {}
            for row in rows:
                category_uuid = row[0]
                category_name = row[1]
                ...
                if _category.get_helper_words():
                    helper_words_insert_query = """
                        INSERT INTO category_helper_words
                            (uuid, helper_word_text, category_uuid, created_at, updated_at)
                        VALUES (%s, %s, %s, %s, %s)
                    """
                    cursor.executemany(
    ...
    async def _delete_by_uuid_and_return_uuid(self, uuid_: UUID) -> UUID | None:
        def _sync_version(_category_uuid: UUID) -> UUID | None:
            set_null_discounted_products_query = """
            ...
            cursor: CursorWrapper
            with connection.cursor() as cursor:
                cursor.execute(sql=set_null_discounted_products_query, params=[_category_uuid])
        ...
        def _sync_version(_helper_word_uuid: UUID) -> Category | None:
            query_ = """
                WITH found_category_uuid AS (
                    SELECT c.uuid
                    FROM categories c
                    JOIN category_helper_words ch ON c.uuid = ch.category_uuid
                    WHERE ch.uuid = %s
                    LIMIT 1
                )
                SELECT
                    ...
                    ch.updated_at
                FROM categories c
                LEFT JOIN category_helper_words ch ON c.uuid = ch.category_uuid
                WHERE c.uuid IN (SELECT uuid FROM found_category_uuid);
            """
            cursor: CursorWrapper
            with connection.cursor() as cursor:
    ...
    async def _update(self, updated_category: Category) -> None:
        def _sync_version(_updated_category: Category) -> None:
            ...
            helper_words_select_query = """
                SELECT uuid, helper_word_text, created_at, updated_at
            ...
            with connection.cursor() as cursor:
                ...
                rows: list[tuple[UUID, str, dt.datetime, dt.datetime]] = cursor.fetchall()
