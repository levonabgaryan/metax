...
class DjangoPostgresqlRetailerRepository(RetailerRepository):
    ...
    async def _delete_by_uuid_and_return_uuid(self, uuid_: UUID) -> UUID | None:
        def _sync_version(_uuid: UUID) -> UUID | None:
            delete_discounted_products_query = """
            ...
            cursor: CursorWrapper
            with connection.cursor() as cursor:
                cursor.execute(sql=delete_discounted_products_query, params=[_uuid])
                cursor.execute(sql=delete_query, params=[_uuid])
    ...
    async def _get_by_uuid(self, uuid_: UUID) -> Retailer | None:
        def _sync_version(_uuid: UUID) -> Retailer | None:
            select_query = """
                SELECT
                    uuid,
                    name,
            ...
            cursor: CursorWrapper
            with connection.cursor() as cursor:
                cursor.execute(sql=select_query, params=[_uuid])
                row = cursor.fetchone()
                if row is None:
                    return None
                return Retailer(
