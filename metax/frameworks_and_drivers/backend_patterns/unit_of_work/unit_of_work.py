...
class UnitOfWork(AbstractUnitOfWork):
    ...
    async def __aenter__(self) -> Self:
        ...
        await sync_to_async(self.__atomic.__enter__, thread_sensitive=True)()
        self.__rolled_back = False
        self.__committed = False
        return self
    ...
    async def __aexit__(
        ...
        if self.__atomic is None:
            return
        try:
            await sync_to_async(self.__atomic.__exit__, thread_sensitive=True)(exc_type, exc_val, exc_tb)
        finally:
            await sync_to_async(close_old_connections, thread_sensitive=True)()

    @override
    async def commit(self) -> None:
        await sync_to_async(transaction.set_rollback, thread_sensitive=True)(False)
        self.__committed = True

    @override
    async def rollback(self) -> None:
        await sync_to_async(transaction.set_rollback, thread_sensitive=True)(True)
