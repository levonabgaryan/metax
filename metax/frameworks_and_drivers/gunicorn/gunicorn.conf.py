...
asgi_loop = "asyncio"  # https://gunicorn.org/reference/settings/?query=gunicorn+--c#asgi_loop

# INFO on the server loggers; formatting is unified in metax_logger.init_logger
# (propagate to root after handlers are cleared there).
loglevel = "info"
# Access line with request_id is logged in RequestIDMiddleware (metax.http.access).
accesslog = None
errorlog = "-"
