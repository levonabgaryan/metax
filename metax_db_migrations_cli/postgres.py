...
def migrate_postgres(
    ...
    timestamp = dt.datetime.now(tz=dt.UTC).strftime("%Y_%m_%d_%H_%M")
    ...
    # Apply all apps (admin, auth, contenttypes, sessions, metax, …). ``migrate metax``
    # alone skips Django contrib tables — ``auth_user`` etc. never get created.
